...
import { resumeSections } from "./resume-lines";
import { activeResume, postingsForCheck, storedChecks } from "./store";
import type { Grade, LineImportance, LineKind, LineSection, ResumeSection } from "./types";
...
export interface CheckedLine {
  ...
  // Where on the resume the evidence came from: what the match is worth rests
  // on this.
  evidenceSection: ResumeSection | null;
}
...
export async function checkDetail(email: string, jobId: string): Promise<CheckDetail> {
  ...
  const lines: CheckedLine[] = posting.lines.map((l) => {
    ...
    return {
      ...
      evidenceSection: g && g.evidence > 0 ? (sections.get(g.evidence) ?? null) : null,
    };
  });

  if (!current) return { status: "pending", lines };
  const score = scoreLines(posting.lines, check.grades, sections);
